...
namespace Z64
{
    ...
    public class Z64ObjectAnalyzerException : Exception
    {
        ...
        public Z64ObjectAnalyzerException(string message)
            : base(message) { }

        public Z64ObjectAnalyzerException(string message, Exception inner)
            : base(message, inner) { }
    ...
    public static class Z64ObjectAnalyzer
    {
            ...
            public class OpCodePattern
            {
                ...
                public static OpCodePattern? Parse(string exp)
                {
                    ...
                    for (int i = 0; i < entries.Count; i++)
                    {
                        if (entries[i] == "*")
                        {
                            pattern.Add(new Tuple<int, List<CmdID>>(0, new List<CmdID>() { id }));
                            continue;
                        }
        ...
        private static bool IsOverlap(int off, List<ReservedRegion> regions) =>
            GetRegionIndex(off, regions) != -1;
        ...
        )
        {
            ...
            if (
                addr.Segmented
                && addr.SegmentId == curSegId
                && ((int)addr.SegmentOff + size) <= totalSize
            )
        ...
        private static Tuple<List<int>, List<ReservedRegion>> FindRegions(
            byte[] data,
            int segmentId,
            Config cfg
        )
        {
            ...
            var codeEnds = Utils.FindData(
                data,
                new byte[] { (byte)CmdID.G_ENDDL, 0, 0, 0, 0, 0, 0, 0 },
                8
            );
            codeEnds.Insert(0, 0);
                    ...
                    switch (op)
                    {
                        ...
                        case CmdID.G_SETTIMG:
                        {
                            ...
                            AddRegion(
                                ...
                                segmentId,
                                settimg.imgaddr,
                                N64Texture.GetTexSize(texels, settimg.siz),
                                data.Length
                            );
                            texels = -1;
                            break;
        ...
        private static List<int> FindDlistEntries(
            List<ReservedRegion> regions,
            List<int> confirmedDlists,
            byte[] data,
            Config cfg
        )
        {
        ...
        public static void FindSkeletons(Z64Object obj, byte[] data, int segmentId)
        {
            ...
            for (int i = 0; i <= data.Length - Z64Object.SkeletonHolder.HEADER_SIZE; i += 4)
            {
                ...
                if (
                    ...
                    && segment.SegmentOff < data.Length
                    && (segment.SegmentOff % 4) == 0
                    && data[i + 5] == 0
                    && data[i + 6] == 0
                    && data[i + 7] == 0
                )
                {
                    ...
                    if (
                        secondLimbSeg.VAddr - firstLimbSeg.VAddr
                    ...
                    if (
                        secondLimbSeg.VAddr - firstLimbSeg.VAddr
                        == Z64Object.SkeletonLimbHolder.SKIN_LIMB_SIZE
                    )
                    {
                        bool limbsTest = true;
                        ...
                        {
                            var limbSeg = new SegmentedAddress(
                                ArrayUtil.ReadUint32BE(limbsData, j)
                            );

                            var value08 = ArrayUtil.ReadUint32BE(
                                data,
                                (int)limbSeg.SegmentOff + 0x8
                            );
                            var segment0C = new SegmentedAddress(
                                ArrayUtil.ReadUint32BE(data, (int)limbSeg.SegmentOff + 0xC)
                            );

                            if (
                                value08 > 255
                                || !(segment0C.SegmentId == segmentId || segment0C.VAddr == 0)
                                || segment0C.VAddr % 4 != 0
                            )
                            {
                                limbsTest = false;
                    ...
                    if (
                        secondLimbSeg.VAddr - firstLimbSeg.VAddr
                        == Z64Object.SkeletonLimbHolder.LOD_LIMB_SIZE
                    )
                    {
                    ...
                    if (
                        i <= data.Length - Z64Object.FlexSkeletonHolder.HEADER_SIZE
                        && obj.IsOffsetFree(i + Z64Object.SkeletonHolder.HEADER_SIZE)
                        && data[i + 8] == nNonNullDlists
                        && data[i + 9] == 0
                        && data[i + 10] == 0
                        && data[i + 11] == 0
                    )
            ...
            for (int i = 0; i <= data.Length - Z64Object.AnimationHolder.HEADER_SIZE; i += 4)
            {
                ...
                )
                {
                    if (
                        !(
                            obj.IsOffsetFree(i)
                            && obj.IsOffsetFree(i + Z64Object.AnimationHolder.HEADER_SIZE - 1)
                        )
                    )
                        continue;
                    ...
                    if (
                        frameDataSeg.SegmentId == segmentId
                        && jointIndicesSeg.SegmentId == segmentId
                        && frameDataSeg.SegmentOff < data.Length
                        && jointIndicesSeg.SegmentOff < data.Length
                    ...
                    )
                    {
                        ...
                        if (
                            frameDataSize < 2
                            || jointIndicesSize < Z64Object.AnimationJointIndicesHolder.ENTRY_SIZE
                        )
                            continue;
                        ...
                        if ((frameDataSize % 2) != 0)
                        {
                            ...
                            Buffer.BlockCopy(
                                data,
                                (int)(
                                    frameDataSeg.SegmentOff + frameDataSize - (frameDataSize % 2)
                                ),
                                possiblePadding,
                                0,
                                frameDataSize % 2
                        ...
                        obj.AddJointIndices(
                            jointIndicesSize / Z64Object.AnimationJointIndicesHolder.ENTRY_SIZE,
                            off: (int)jointIndicesSeg.SegmentOff
                        );
                    }
                }
            }
                ...
                if (frameCount > 0 && data[i + 2] == 0 && data[i + 3] == 0)
                {
                    ...
                    if (animationSeg.SegmentId == 7 && animationSeg.SegmentOff < linkAnimetionSize)
                    {
                        ...
                        if (
                            !(
                                obj.IsOffsetFree(i)
                                && obj.IsOffsetFree(i + Z64Object.PlayerAnimationHolder.SIZE - 1)
                            )
                        )
                            continue;
        ...
        public static void FindCollisionData(Z64Object obj, byte[] data, int segmentId)
        {
            ...
            for (int i = 0; i < data.Length - Z64Object.ColHeaderHolder.COLHEADER_SIZE; i += 4)
            {
                ...
                )
                {
                    ...
                    var surfaceTypesSeg = new SegmentedAddress(
                        ArrayUtil.ReadUint32BE(data, i + 0x1C)
                    );
                    var camDataSeg = new SegmentedAddress(ArrayUtil.ReadUint32BE(data, i + 0x20));
                    var waterBoxesSeg = new SegmentedAddress(
                        ArrayUtil.ReadUint32BE(data, i + 0x28)
                    );
                    ...
                    Func<SegmentedAddress, int, int, bool, bool> validSegmentAddr = (
                        seg,
                        num,
                        align,
                        allowNull
                        ...
                        (
                            (
                                ...
                                && seg.SegmentOff < data.Length
                                && obj.IsOffsetFree((int)seg.SegmentOff)
                                && seg.SegmentOff % align == 0
                            ) || (allowNull && num == 0 && seg.VAddr == 0)
                        );

                    if (
                        validSegmentAddr(verticesSeg, nVertices, 2, false)
                    ...
                    )
                    {
                        ...
                        if (surfaceTypesSize != 0 && surfaceTypesPadLen != 0)
                        {
                            ...
                            Buffer.BlockCopy(
                                data,
                                (int)surfaceTypesSeg.SegmentOff + surfaceTypesSize,
                                possiblePadding,
                                0,
                                surfaceTypesPadLen
                            );
                        ...
                        if (camDataSeg.VAddr != 0 && camDataSize != 0 && camDataPadLen != 0)
                        {
                            ...
                            Buffer.BlockCopy(
                                data,
                                (int)surfaceTypesSeg.SegmentOff + camDataSize,
                                possiblePadding,
                                0,
                                camDataPadLen
                            );
                        ...
                        colHeader.VerticesHolder = obj.AddCollisionVertices(
                            nVertices,
                            off: (int)verticesSeg.SegmentOff
                        );
                        colHeader.PolygonsHolder = obj.AddCollisionPolygons(
                            nPolygons,
                            off: (int)polygonsSeg.SegmentOff
                        );
                        ...
                        if (waterBoxesSeg.VAddr != 0)
                            colHeader.WaterBoxHolder = obj.AddWaterBoxes(
                                nWaterBoxes,
                                off: (int)waterBoxesSeg.SegmentOff
                            );
        ...
        public static void FindMaterialAnimations(
            Z64Object obj,
            byte[] data,
            int segmentId,
            Dictionary<int, SegmentedTextureAttrs> textureLoads
        )
        {
            ...
            for (int i = 0; i <= data.Length - Z64Object.MatAnimHeaderHolder.SIZE; i += 4)
            {
                ...
                )
                {
                    ...
                    if (
                        paramsSeg.SegmentId != segmentId
                        || paramsSeg.SegmentOff >= data.Length
                    ...
                    { // Tex Scroll
                        ...
                        if (
                            paramsSeg.SegmentOff + Z64Object.MatAnimTexScrollParamsHolder.SIZE
                                > data.Length
                            || !obj.IsOffsetFree(
                                (int)paramsSeg.SegmentOff
                                    + Z64Object.MatAnimTexScrollParamsHolder.SIZE
                                    - 1
                            )
                        )
                            continue;
                    ...
                    { // Tex Cycle
                        ...
                        // Check if params fits inside the object
                        if (
                            paramsSeg.SegmentOff + Z64Object.MatAnimTexCycleParamsHolder.SIZE
                                > data.Length
                            || !obj.IsOffsetFree(
                                (int)paramsSeg.SegmentOff
                                    + Z64Object.MatAnimTexCycleParamsHolder.SIZE
                                    - 1
                            )
                        )
                            continue;
                        ...
                        var textureListSeg = new SegmentedAddress(
                            ArrayUtil.ReadUint32BE(paramsData, 4)
                        );
                        var textureIndexListSeg = new SegmentedAddress(
                            ArrayUtil.ReadUint32BE(paramsData, 8)
                        );
                        ...
                        if (
                            !(
                                paramsData[2] == 0
                                && paramsData[3] == 0
                                && textureListSeg.SegmentId == segmentId
                                && textureListSeg.SegmentOff < data.Length
                                && textureIndexListSeg.SegmentId == segmentId
                                && textureIndexListSeg.SegmentOff < data.Length
                        ...
                        Buffer.BlockCopy(
                            data,
                            (int)textureIndexListSeg.SegmentOff,
                            textureIndices,
                            0,
                            keyFrameLength
                        );
                        ...
                        {
                            var textureSegment = new SegmentedAddress(
                                ArrayUtil.ReadUint32BE(textureListData, j)
                            );
                            if (
                                !(
                                    textureSegment.SegmentId == segmentId
                                    && textureSegment.SegmentOff < data.Length
                                )
                            )
                            {
                                texturesOk = false;
                        ...
                        if (textureLoads.TryGetValue(texSegment, out texAttrs))
                        {
                            for (int j = 0; j < nTextures * 4; j += 4)
                            {
                                ...
                                obj.AddTexture(
                                    ...
                                    texAttrs.Height,
                                    texAttrs.Fmt,
                                    off: (int)textureSegment.SegmentOff
                                );
                            }
                        }

                        obj.AddMatAnimTextureIndexList(
                            keyFrameLength,
                            off: (int)textureIndexListSeg.SegmentOff
                        );
                    ...
                    { // Color
                        ...
                        // Check if params fits inside the object
                        if (
                            paramsSeg.SegmentOff + Z64Object.MatAnimColorParamsHolder.SIZE
                                > data.Length
                            || !obj.IsOffsetFree(
                                (int)paramsSeg.SegmentOff
                                    + Z64Object.MatAnimColorParamsHolder.SIZE
                                    - 1
                            )
                        )
                            continue;
                        ...
                        var primColors = new SegmentedAddress(
                            ArrayUtil.ReadUint32BE(paramsData, 4)
                        ); // size 5 per elem
                        var envColors = new SegmentedAddress(ArrayUtil.ReadUint32BE(paramsData, 8)); // size 4 per elem
                        var keyFrames = new SegmentedAddress(
                            ArrayUtil.ReadUint32BE(paramsData, 0xC)
                        ); // size 2 per elem
                        ...
                        if (
                            !(
                                primColors.SegmentId == segmentId
                                && primColors.SegmentOff < data.Length
                            )
                        )
                            continue;
                        if (
                            !obj.IsOffsetFree((int)primColors.SegmentOff)
                        ...
                        if (
                            !(
                                (
                                    envColors.SegmentId == segmentId
                                    && envColors.SegmentOff < data.Length
                                )
                                || envColors.VAddr == 0
                            )
                        )
                        ...
                        if (
                            ...
                            && (
                                ...
                                || !obj.IsOffsetFree(
                                    (int)envColors.SegmentOff + arrayLengths * 4 - 1
                                )
                            )
                        )
                            continue;
                        ...
                        if (
                            !(
                                (
                                    keyFrames.SegmentId == segmentId
                                    && keyFrames.SegmentOff < data.Length
                                ) || (paramsType == 2 && keyFrames.VAddr == 0)
                            )
                        )
                            continue;
                        if (
                            ...
                            && (
                                ...
                                || !obj.IsOffsetFree(
                                    (int)keyFrames.SegmentOff + arrayLengths * 2 - 1
                                )
        ...
        public static List<string> AnalyzeDlists(Z64Object obj, byte[] data, int segmentId)
        {
            ...
            Dictionary<int, SegmentedTextureAttrs> segmentedTextureLoads =
                new Dictionary<int, SegmentedTextureAttrs>();
            ...
            for (int i = 0; i < obj.Entries.Count; i++)
            {
                ...
                if (entry.GetEntryType() == Z64Object.EntryType.DList)
                    ...
                    obj.Entries[i] = new Z64Object.UnknowHolder(
                        $"unk_{obj.OffsetOf(entry):X8}",
                        entry.GetData()
                    );
            }
                    ...
                    switch (op)
                    {
                        ...
                        case CmdID.G_VTX:
                        {
                            ...
                            if (addr.Segmented && addr.SegmentId == segmentId)
                            {
                                ...
                                {
                                    errors.Add(
                                        $"Error in Dlist 0x{new SegmentedAddress(segmentId, dlist).VAddr:X8} : {ex.Message}"
                                    );
                                }
                            }
                            break;
                        ...
                        case CmdID.G_SETTILESIZE:
                        {
                            ...
                            if (addr.Segmented && addr.SegmentId == segmentId)
                            {
                                try
                                {
                                    var tex = obj.AddTexture(
                                        (int)(settilesize.lrs.Float() + 1),
                                        (int)(settilesize.lrt.Float() + 1),
                                        N64Texture.ConvertFormat(lastFmt, lastSiz),
                                        off: (int)addr.SegmentOff
                                    );
                            ...
                            { // enumerate texture loads from segment 8+ for animated material detection
                                ...
                                if (
                                    segmentedTextureLoads.TryGetValue(addr.SegmentId, out lastAttrs)
                                )
                                {
                                    if (
                                        lastAttrs.Fmt != attrs.Fmt
                                        || lastAttrs.Width != attrs.Width
                                        || lastAttrs.Height != attrs.Height
                                    )
                                    {
                                        ...
                                        errors.Add(
                                            $"Error in Dlist 0x{new SegmentedAddress(segmentId, dlist).VAddr:X8} : Usage of segment 0x{addr.SegmentId:X2} is ambiguous"
                                        );
                                    }
                                }
                                else
                        ...
                        case CmdID.G_LOADTLUT:
                        {
                            ...
                            if (addr.Segmented && addr.SegmentId == segmentId)
                            {
                                try
                                {
                                    lastTlut = obj.AddTexture(
                                        w,
                                        (loadtlut.count + 1) / w,
                                        N64Texture.ConvertFormat(
